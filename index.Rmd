...
data <- sample_frac(nbastats)  #randomly order rows
...
for (i in 1:k) {
    ...
    # train model
    fit <- knn3(Citzenship == "True" ~ Salary + Weight + Points + Blocks + Steals + Assists + Rebounds + Age + MP + G, data = test)  ### SPECIFY THE LOGISTIC REGRESSION MODEL FIT TO THE TRAINING SET HERE
...
linearfit<-lm(Points~.,data=nbaclusterdata) #predict mpg from all other variables
linearyhat<-predict(linearfit) #predicted mpg
...
data<-nbaclusterdata[sample(nrow(nbaclusterdata)),] #randomly order rows
folds<-cut(seq(1:nrow(nbaclusterdata)),breaks=k,labels=F) #create folds
diags<-NULL
set.seed(1234)
k=5 #choose number of folds
data<-nbaclusterdata[sample(nrow(nbaclusterdata)),] #randomly order rows
